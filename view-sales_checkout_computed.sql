...
AS WITH initial_table AS (
         SELECT sales_checkout.checkout_id,
            ...
            sales_order.tax,
            sales_order.status,
            sales_checkout.billing_address_id
           FROM sales_checkout
             JOIN sales_order ON sales_order.checkout_id = sales_checkout.checkout_id
          GROUP BY sales_checkout.checkout_id, sales_checkout.customer_id, sales_checkout.date_add, sales_checkout.date_upd, sales_checkout.discount_amount, sales_checkout.discount_code, sales_checkout.status, sales_checkout.cart_status, sales_checkout.shipping_fee, sales_checkout.shipping_id, sales_order.order_id, sales_order.status, sales_order.tax, sales_checkout.billing_address_id
        ), delivery_flags AS (
         SELECT initial_table.checkout_id,
            ...
            initial_table.billing_address_id,
 ...
 SELECT delivery_flags.checkout_id,
    ...
    sum(delivery_flags.tax) AS total_tax,
    delivery_flags.billing_address_id
   FROM delivery_flags
  GROUP BY delivery_flags.checkout_id, delivery_flags.customer_id, delivery_flags.checkout_status, delivery_flags.date_add, delivery_flags.date_upd, delivery_flags.discount_amount, delivery_flags.discount_code, delivery_flags.cart_status, delivery_flags.shipping_fee, delivery_flags.shipping_id, delivery_flags.billing_address_id;